...
class Evaluator:
    ...
    def webvpn_login(self, username, password):
        ...
        elif response.json()['error'] == 'INVALID_ACCOUNT':
    ...
    def jw_login(self, username, password):
        ...
        response = self.session.get(publickey_url, headers=self.headers)
        # 返回的不是json而是页面时
        if 'text/html' in response.headers.get('Content-Type'):
            self.log('wevpn登录信息已过期')
            raise CustomException(Result.failed(msg='wevpn登录信息已过期，请回到第一步重试'))
    ...
    def run(self, username, password, content, mode):
